...
async def generate_cover(thumbnail, title, userid, ctitle):
    ...
    image2 = Image.open("BruTalxD/BadnamMusic.png")
    ...
    font = ImageFont.truetype("BruTalxD/finalfont.ttf", 60)
    font2 = ImageFont.truetype("BruTalxD/finalfont.ttf", 70)     
    ...
    if replied:
        ...
        if len(m.command) < 2:
         await m.reply_photo(
                    ...
                    [
                        [
                            InlineKeyboardButton("~ Channel ~", url=f"https://t.me/XCodeBots"),
                            InlineKeyboardButton("~ Support ~", url=f"https://t.me/XCodeSupport")
                        ],
                        [
                            InlineKeyboardButton("✒ Close", callback_data="cls")
                        ]
                ...
                if abhi == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await m.reply_photo(
                                photo=image,
                                caption=f"🏷 **°Nᴀᴍᴇ:** [{songname[:22]}]({url})\n**⏱ °Dᴜʀᴀᴛɪᴏɴ:** `{duration}`\n💡 **°Sᴛᴀᴛᴜꜱ:** `Playing`\n🎧 **°RᴇQᴇꜱᴛᴇᴅ Bʏ:** {requester}",
                                reply_markup=keyboard,
                            )
